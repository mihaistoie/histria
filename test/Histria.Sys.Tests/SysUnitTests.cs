...
namespace Histria.Sys.Tests
{
    ...
    public class SysUnitTests
    {
        ...
        public void PropertyChangeStack()
        {
            ...
            string g4 = Guid.NewGuid().ToString("N");
            stack.Push(string.Format("{0}.delails.{1}.UnitPrice", g1, g2));
            stack.Push(string.Format("{0}.delails.{1}.Price", g1, g2));
            stack.Push(string.Format("{0}.delails.{1}.UnitPrice", g1, g4));
            stack.Push(string.Format("{0}.delails.{1}.Price", g1, g4));
            ...
            Assert.AreEqual(true, stack.Has(search, "delails.Price"), "Found");
            search = string.Format("{0}", g1);
            Assert.AreEqual(false, stack.Has(search, "delails.M"), "Not Found");
