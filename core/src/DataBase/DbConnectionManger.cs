...
namespace Sikia.DataBase
{
    ...
    public class DbConnectionManger
    {
        ...
        public static DbConnectionManger LoadConfig(ApplicationConfig config)
        {
            DbConnectionManger conn = new DbConnectionManger();
            if (config == null)
                config = new ApplicationConfig();
            conn.Load(config.Databases);
            return conn;
        }

        public static DbConnectionManger Instance(ApplicationConfig config)
        {
            if (instance == null)
            {
                lock (syncRoot)
